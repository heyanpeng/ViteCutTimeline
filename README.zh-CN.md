...
- 完全受控的数据模型：`editorData` / `currentTime` / `playing` 由外部管理
- 完整事件回调：移动、裁剪、光标拖拽、点击与双击等交互可外部接管
- 轨道/片段拖拽，支持跨轨道移动与可选“拖拽插入新轨道”
- 左右裁剪（Trim），支持吸附到片段边缘与时间刻度
- 支持动态时间轴时长（内容末尾 + 固定留白）
- 支持自定义片段渲染（`getActionRender` 与拖拽预览渲染）
- 轨道状态支持：锁定/隐藏/静音/删除逻辑可由宿主应用控制
- 大数据量场景下的可见区域虚拟化渲染
- 播放头驱动编辑
